...
public class CustomerController : ControllerBase
{
    ...
    //        //Get peginated data
    //        var customers = await customerService.GetPaginatedCustomers(pageNumberValue, pageSizeValue);
    ...
    public async Task<IActionResult> Get(int id)
    {
        ...
        catch (Exception ex)
        {
            ...
            logger.LogError(ex, $"An error occurred while retrieving the customer");
            return StatusCode(StatusCodes.Status500InternalServerError, ex.Message);
        }
    }
    ...
    public async Task<IActionResult> Create(CustomerViewModel model)
    {
        ...
        {
            string message = "";
            if (await customerService.IsExists("Email", model.Email))
            {
            ...
            try
            {
                var data = await customerService.Create(model);
                return Ok(data);
            }
            catch (Exception ex)
            {
                logger.LogError(ex, $"An error occurred while adding the customer");
    ...
    public async Task<IActionResult> Edit(CustomerViewModel model)
    {
        ...
        {
            string message = "";
            if (await customerService.IsExistsForUpdate(model.Id, "Email", model.Email))
            {
                message = $"The customer email- '{model.Email}' already exists";
                return StatusCode(StatusCodes.Status400BadRequest, message);
            }
            ...
            {
                logger.LogError(ex, $"An error occurred while updating the customer");
